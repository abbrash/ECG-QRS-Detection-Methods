% Main script for QRS detection using zero-crossing method
...
mainDir = fileparts(which('main_crossing.m'));
if isempty(mainDir)
    error('main_crossing.m not found in the MATLAB path. Please ensure it exists and is in the MATLAB path.');
end
...
% Detect QRS complexes using the zero-crossing method
[qrs_peaks, sensitivity, positive_predictivity] = detect_qrs_crossing(signal, Fs, ann);
